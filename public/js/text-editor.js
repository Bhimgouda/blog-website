...
if (titleCount.textContent >= 25 && titleCount.textContent <= 70)
  titleClass.add("valid");
...
const descriptionCount = document.querySelector(
  `[data-for="description"] span`
);
const descriptionClass = document.querySelector(
  `[data-for="description"]`
).classList;
if (descriptionCount.textContent >= 120 && descriptionCount.textContent <= 200)
  descriptionClass.add("valid");

validateInputChar(description, descriptionCount, 120, 200, descriptionClass);
...
if (taglineCount.textContent >= 40 && taglineCount.textContent <= 60)
  taglineClass.add("valid");
...
if (seoTitleCount.textContent >= 40 && seoTitleCount.textContent <= 60)
  seoTitleClass.add("valid");
...
// if (keywordsCount.textContent >= 2 && keywordsCount.textContent <= 5)
//   keywordsClass.add("valid");
...
form.onsubmit = function (e) {
  ...
  if (heroImageClass) {
    // for new article and edit
    if (
      !(
        descriptionClass.contains("valid") &&
        // taglineClass.contains("valid") &&
        titleClass.contains("valid") &&
        // seoTitleClass.contains("valid") &&
        // keywordsClass.contains("valid") &&
        heroImageClass.classList.contains("valid")
      )
    ) {
      e.preventDefault();
    }
  } else {
    // only for edit article
    if (
      !(
        descriptionClass.contains("valid") &&
        // taglineClass.contains("valid") &&
...
function tooltipBtn(e) {
  ...
  document.querySelectorAll("#tooltip-controls button").forEach((btn) => {
    if (this.classList.contains("tooltip-1")) {
      ...
      if (btn.id.includes("header")) btn.classList.remove("active");
    }
  });

  this.classList.toggle("active");
  if (this.classList.contains("active")) quill.format(this.id, true);
...
document.querySelector("#header-2").addEventListener("click", function (e) {
  e.preventDefault();
  document.querySelectorAll("#tooltip-controls button").forEach((btn) => {
    if (btn.id !== this.id) btn.classList.remove("active");
  });
  this.classList.toggle("active");
  if (this.classList.contains("active")) quill.format("header", 1);
  else quill.format("header", false);
});
...
document.querySelector("#header-3").addEventListener("click", function (e) {
  ...
  this.classList.toggle("active");
  if (this.classList.contains("active")) quill.format("header", 2);
  else quill.format("header", false);
});

document
  .querySelector("#show-controls")
